...
def plot_sun(image,timestamp):
    global LATITUDE,LONGITUDE
    ground = earth + Topos(LATITUDE, LONGITUDE, elevation_m = ELEVATION)
    alt,az,dist = (sun - ground).at(timestamp).altaz()
    sun_angular_radius = np.arcsin(6.957e5/dist.km)*180/np.pi #sun radius
    crosshair = plot_stamp(az.degrees,alt.degrees,plt.imread('crosshair.png'),5)
    return plot_stamp(az.degrees, alt.degrees, image, sun_angular_radius),crosshair
...
def update_plot(timestamp):
    ...
    sun_stamp = plot_sun(SUN_IMAGE,timestamp)
    map_marker = update_map()
    if len(plotted_objects) == 0: #just executes on the first run
        iss_extent = iss_trail_and_stamp[1].get_extent()
        if target == moon:
            target_extent = moon_stamp_and_mask[0].get_extent()
        if target == sun:
            target_extent = sun_stamp[0].get_extent()
        iss_xvals = iss_extent[0:2]
        iss_yvals = iss_extent[2:4]
        target_xvals = target_extent[0:2]
        target_yvals = target_extent[2:4]

        minx = min(*iss_xvals,*target_xvals)-10
        maxx = max(*iss_xvals,*target_xvals)+10
        miny = min(*iss_yvals,*target_yvals)-10
        maxy = max(*iss_yvals,*target_yvals)+10

        axes[0].set_xlim(minx,maxx)
        axes[0].set_ylim(miny,maxy)
    plotted_objects = [starfield,*iss_trail_and_stamp,*moon_stamp_and_mask,*sun_stamp,map_marker]
def update_map():
    global LATITUDE,LONGITUDE
